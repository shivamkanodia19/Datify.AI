...
type GameAction =
  ...
  | { type: 'SET_SESSION_VERSION'; payload: number };
...
function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    ...
    case 'SET_SESSION_VERSION':
      return {
        ...state,
        sessionVersion: action.payload,
      };
...
export function useSwipeGameState(initialDeck: Place[]) {
  ...
  const actions = {
    ...
    setSessionVersion: useCallback((version: number) => {
      dispatch({ type: 'SET_SESSION_VERSION', payload: version });
    }, []),
  };
